...
public class HomeLoanService {
    ...
    public LoanApplication applyForHomeLoan(Account account, double propertyPrice) {
        double downPaymentPercentage = 0.15;
        if (propertyPrice >= 150_000.00) {
            downPaymentPercentage = 0.20;
        }

        double downPaymentAmount = propertyPrice * downPaymentPercentage;

        boolean isGranted = account.balance() >= downPaymentAmount;
